...
class SparkContext(config: SparkConf) extends Logging with ExecutorAllocationClient {
  ...
  // (BJH) Saving optimized LoP
  private[spark] var _optLoP: ArrayBuffer[Int] = ArrayBuffer.empty[Int]
  private[spark] def setLoP(LoP: Int) = { _optLoP += LoP }
  private[spark] def getLoP(test: Int): Int = _optLoP(test)

  // (BJH) Saving optimized DoP
  private[spark] var _optDoP: ArrayBuffer[Int] = ArrayBuffer.empty[Int]
  private[spark] def setDoP(DoP: Int) = { _optDoP += DoP }
  private[spark] def getDoP(index: Int): Int = _optDoP(index)

  private[spark] var incDoP: Array[Int] = Array()
  private[spark] var newCurrentDoP: Array[Int] = Array()
  private[spark] var taskFinished: Array[Int] = Array()
  private[spark] var taskHistory: Array[Double] = Array()
  private[spark] var spillness: Array[Boolean] = Array()
  private[spark] var numTaskInStage: Int = 0
  private[spark] var currentDoP: Int = 0
  private[spark] var totalCore: Int = 0
  private[spark] var totalExecutor: Int = 0
  private[spark] var stageId: Int = 0
  private[spark] var taskFetched: Int = 0
  private[spark] var jobNoti: Int = 1
  private[spark] var maxPartitions: Int = 0
  var persistFlag = false
  ...
  private[spark] def getCallSite(): CallSite = {
    ...
    CallSite(
      ...
      Option(getLocalProperty(CallSite.METHOD_NAME)).getOrElse(callSite.methodName)
    )
  }
  ...
  def runJob[T, U: ClassTag](
      ...
      func: Iterator[T] => U,
      partitions: Seq[Int]): Array[U] = {
    //logError(",ARC runJob 2")
    if (beforeJob == 0) {
      _optLoP = ArrayBuffer.empty[Int]
      _optDoP = ArrayBuffer.empty[Int]
      PredictLoPDoP(rdd)
      jobCnt += 1
    }
    beforeJob = 2
  ...
  def PredictLoPDoP[T](rdd: RDD[T]) = {
    val confInput = conf.getOption("spark.input.size")
      .map(Utils.memoryStringToMb)

    val totalSize = (confInput match {
      case Some(x: Int) => x
      case _ => 0
    })
    
    val inputSize = totalSize 

    var shuffleArr = Array.fill(rdd.id + 1)(0)
    var paramArr = Array.fill(rdd.id + 1)(0.0)
    var dataTrans = Array.fill(rdd.id + 1)(0.0)
    var LoPArr = Array.fill(rdd.id + 1)(0)
    var funclist = Array.fill(rdd.id + 1)(Array("missing", -1))

    for (dep <- rdd.dependencies) {
      var len = rdd.dependencies.length
      len match {
        case 0 =>
          funclist(rdd.id) = Array(rdd.creationSite.methodName, -1)
        case 1 =>
          funclist(rdd.id) = Array(rdd.creationSite.methodName, dep.rdd.id)
        case 2 =>
          var numParent = 1
          var tempArr = Array(rdd.creationSite.methodName, -1, -1)
          val frontDeps = rdd.dependencies.head
          val lastDep = rdd.dependencies.last
          tempArr(1) = frontDeps.rdd.id
          tempArr(2) = lastDep.rdd.id
          funclist(rdd.id) = tempArr
      }
      ...
      fixDep(dep.rdd, funclist, shuffleArr)
    }

    var index = 0
    for (rddFunc <- funclist) {
      paramArr(index) = paramGet(rddFunc(0).toString)
      index += 1
    }

    dataTrans = dataTransWeight(inputSize, paramArr, funclist)
    LoPArr = calculateLoP(dataTrans, shuffleArr)
   
    rdd.newNumPartitions = LoPArr(rdd.id)
    for (dep <- rdd.dependencies) {
      fixNewDep(dep.rdd, LoPArr)
    }
    ...
    for (dep <- rdd.dependencies) {
      ...
      dep match {
        case shuffleDep: ShuffleDependency[_, _, _] =>
          val setNew = dep.setNew(LoPArr(rdd.id - 1))
          newPartDep(dep.rdd, LoPArr)
        case narrowDep: NarrowDependency[_] =>
          newPartDep(dep.rdd, LoPArr)
      }
    }

    logError(",ARC optLoP, optDoP: " + _optLoP + ", " +_optDoP)
  }

  def getExecutorMemory(sconf: SparkConf): Long = {
    val confMemory = sconf.getOption("spark.executor.memory")
      .map(Utils.memoryStringToMb)
    ...
    val reserved = 300
    val fraction = sconf.getDouble("spark.memory.fraction", 0.75)

    ((totalMemory - reserved) * fraction).toLong
  }

  def LoPModel(dataSize: Double, stageStart: Int): Int = {
    // (BJH) Get param to calculate optDoP
    var currentMem = getExecutorMemory(conf)
    if (persistFlag) {
      currentMem = currentMem / 2
    }
    val confExecutor = conf.getOption("spark.total.executor.number")
...
//    val gcconst = 0
    ...
    // (BJH) Calculate optLoP
//    val a = gcconst
    val b = dataSize
    val c = currentMem

    logError(s",ARC b: $b, c: $c")
    val confCore = conf.getOption("spark.total.core.number")
    ...
    //logError(",ARC end")
    if (minLoP < (totalExecutor * calDoP)) {
      minLoP = totalExecutor * calDoP
      minDoP = calDoP
    }

    if (minLoP > defaultParallelism * 8) {
      minLoP = defaultParallelism
      minDoP = calDoP
    }
  ...
  def calculateLoP(dataTrans: Array[Double], shuffleArr: Array[Int]): Array[Int] = {
    var index = 0
    var stageStart = 0
    var LoPArr = Array.fill(dataTrans.length)(0)
    var maxDataSize = Array.fill(1)(0.0)

    for(i <- dataTrans) {
      if(shuffleArr(index) == 1) {
        maxDataSize = Array.fill(index - stageStart)(0.0)
        for(j <- stageStart to index-1) {
          maxDataSize(j - stageStart) = dataTrans(j)
        }

        val maxInStage = maxDataSize.reduceLeft(_ max _)
        var LoP = LoPModel(maxInStage, stageStart)

        if(stageStart == 0) {
          for(j <- stageStart to index-1) {
            LoPArr(j) = defaultParallelism
          }
        } else {
          for(j <- stageStart to index-1) {
            LoPArr(j) = LoP
  ...
  def paramGet(func: String): Double = {
    var weight = 0.0
    if(func == "textFile") { weight = 3 }
    else if(func == "map") { weight = 1.42 }
    else if(func == "flatMap") { weight = 2.04 }
    else if(func == "reduceByKey") { weight = 0.03 }
    else if(func == "groupByKey") { weight = 0.43 }
    else if(func == "combineByKey") { weight = 0.10 }
    else if(func == "join") { 
      multiRdd += 1
      if(multiRdd == 1) { weight = 0.85 }
      else if(multiRdd == 2) { weight = 1 }
      else { multiRdd = 0; weight = 1 }
    }
    else if(func == "mapValues") { weight = 1.07 }
    else if(func == "distinct") {
      multiRdd += 1
      if(multiRdd == 1) { weight = 0.98 }
      else if(multiRdd == 2) { weight = 1 }
      else { multiRdd = 0; weight = 1 }
    }
    else if(func == "sortByKey") { weight = 1 }
    else if(func == "sequenceFile") { weight = 3 }
    else if(func == "zip") { weight = 1.16 }
    else if(func == "zipWithIndex") { weight = 0.97 }
    else if(func == "mapPartitionsWithIndex") { weight = 0.002 }
    else if(func == "mapPartitions") { weight = 0.06 }
    else if(func == "filter") { weight = 0.19 }
    else if(func == "randomSplit") { weight = 1.01 }
    else if(func == "takeSample") { weight = 0.07 }
    else { weight = 1 }
  ...
  def dataTransWeight(
      inputSize: Double,
      paramArr: Array[Double],
      funclist: Array[Array[Any]]): Array[Double] = {
    var index = 0
    var dataTrans = Array.fill(paramArr.length)(0.0)
    for (weight <- paramArr) {
      val tmpArr = funclist(index)
      if(tmpArr.length > 2) {
//        var maxRdd = Array.fill(tmpArr.length - 1)(0.0)
//        for(i <- 1 to tmpArr.length - 1) {
//          maxRdd(i-1) += dataTrans(tmpArr(i).asInstanceOf[Int])
//        }
//        dataTrans(index) = maxRdd.reduceLeft(_ max _)
        dataTrans(index) = (dataTrans(tmpArr(1).asInstanceOf[Int]) + dataTrans(tmpArr(2).asInstanceOf[Int])) * paramArr(index)
      } else if(index == 0) {
        dataTrans(index) = inputSize
      } else {
        dataTrans(index) = dataTrans(index - 1) * paramArr(index)
      }
      index += 1
    }
  ...
  def fixDep[T](rdd: RDD[T], funclist: Array[Array[Any]], shuffleArr: Array[Int]): Unit = {
    for (dep <- rdd.dependencies) {
      var len = rdd.dependencies.length
      len match {
        case 0 =>
  ...
  def fixNewDep[T](rdd: RDD[T], LoPArr: Array[Int]): Unit = {
    rdd.newNumPartitions = LoPArr(rdd.id)
    for (dep <- rdd.dependencies) {
      fixNewDep(dep.rdd, LoPArr)
    }
  ...
  def newPartDep[T](rdd: RDD[T], LoPArr: Array[Int]): Unit = {
    val length = rdd.partitions.length

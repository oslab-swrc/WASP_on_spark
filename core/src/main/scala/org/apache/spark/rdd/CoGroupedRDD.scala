...
class CoGroupedRDD[K: ClassTag](
  ...
  override def getNewPartitions(reset: Int): Array[Partition] = {
    val newPart = if(reset > 0) {
      new HashPartitioner(reset).asInstanceOf[Partitioner]
    } else {
      new HashPartitioner(part.numPartitions).asInstanceOf[Partitioner]
    }
    val array = new Array[Partition](newPart.numPartitions)
    for (i <- 0 until array.length) {
      // Each CoGroupPartition will have a dependency per contributing RDD
      array(i) = new CoGroupPartition(i, rdds.zipWithIndex.map { case (rdd, j) =>
        // Assume each RDD contributed a single dependency, and get it

  ...
  with Serializable
{
  ...
  def join[W](other: RDD[(K, W)]): RDD[(K, (V, W))] = self.withScope {
    join(other, new HashPartitioner(self.sparkContext.defaultParallelism))
  }
  ...
  def saveAsHadoopDataset(conf: JobConf): Unit = self.withScope {
    ...
    val writeToFile = (context: TaskContext, iter: Iterator[(K, V)]) => {
      ...
      val (outputMetrics, bytesWrittenCallback) = initHadoopOutputMetrics(context)
      var ARChadoopTime: Long = 0
      val ARCstart = System.currentTimeMillis()
      ...
      writer.commit()
      ARChadoopTime += System.currentTimeMillis() - ARCstart
      logError(",ARC hadoop time," + ARChadoopTime)
      bytesWrittenCallback.foreach { fn => outputMetrics.setBytesWritten(fn()) }
      outputMetrics.setRecordsWritten(recordsWritten)
    }

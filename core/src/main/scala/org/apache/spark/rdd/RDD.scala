...
//  protected def getDependencies: Seq[Dependency[_]] = deps
  ...
  @transient var partitions_ : Array[Partition] = null
   ...
   * RDD is checkpointed or not.
   */
  @transient var newNumPartitions = 0
  protected def getNewPartitions(reset: Int): Array[Partition]
  final def partitions: Array[Partition] = {
    checkpointRDD.map(_.partitions).getOrElse {
      if (partitions_ == null) {
        if (newNumPartitions > 0) {
          partitions_ = getNewPartitions(newNumPartitions)
        } else {
  ...
  def distinct(): RDD[T] = withScope {
    distinct(sparkContext.defaultParallelism)
  }

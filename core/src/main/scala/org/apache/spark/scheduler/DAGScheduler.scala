...
class DAGScheduler(
  ...
  private def newOrUsedShuffleStage(
    ...
    shuffleDep.partitioner = new HashPartitioner(sc.getLoP(index)).asInstanceOf[Partitioner]
    index += 1
  ...
  private def submitMissingTasks(stage: Stage, jobId: Int) {
    ...
    stage match {
      case s: ShuffleMapStage =>
        //(BJH) Initialize taskQueue to 0 or optDoP
        var optDoP = {
          if (s.id > 0) {
            sc.getDoP(index - 2)
          } else {
        ...
        sc.incDoP = Array.fill(16)(1)
        sc.newCurrentDoP = Array()
        sc.taskFinished = Array.fill(16)(0)
        sc.taskHistory = Array.fill(16)(0.0)
        sc.spillness = Array.fill(16)(false)
        sc.currentDoP = optDoP
        sc.stageId = s.id
        sc.taskFetched = 0
        sc.jobNoti = 1
        if (index == 2) {
        ...
        sc.maxPartitions = s.numPartitions

        logError(",ARC stage start")
      ...
      case s: ResultStage =>
        ...
        sc.maxPartitions = s.rdd.partitions.length

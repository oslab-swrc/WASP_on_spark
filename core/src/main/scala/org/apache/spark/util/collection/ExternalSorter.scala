...
private[spark] class ExternalSorter[K, V, C](
  ...
  if (context.partitionId() < 64) {
    SparkEnv.lowerbound = 0L
    SparkEnv.kvThreshold = 0L
  }
  ...
  def insertAll(records: Iterator[Product2[K, V]]): Unit = {
    ...
    if (shouldCombine) {
      ...
      while (records.hasNext) {
        addElementsRead()
        kv = records.next()
        if (kvcount == 0 && SparkEnv.kvThreshold < 1) {
          SparkEnv.kvThreshold = (1024 * 1024 * 1024 / 2 / 2 / 4 / 16 /
            SizeEstimator.estimate(kv.asInstanceOf[AnyRef])).asInstanceOf[Long] * 5
          SparkEnv.lowerbound = (1024 * 1024 * 1024 / 2 / 2 / 4 / 128 /
            SizeEstimator.estimate(kv.asInstanceOf[AnyRef])).asInstanceOf[Long] * 5
          logError("ARC kvsize1: " + SparkEnv.kvThreshold)
        }
        kvcount += 1
   ...
   */
  private[this] var spillRunTime: Long = 0L
  private[this] var kvflag: Boolean = false

  private def maybeSpillCollection(usingMap: Boolean): Unit = {
    def computeGcTime(): Long = {
      ManagementFactory.getGarbageCollectorMXBeans.asScala.map(_.getCollectionTime).sum
    }
    if (SparkEnv.kvThreshold > 0) {
      kvflag = SparkEnv.kvThreshold < kvcount
    }
    var startGcTime: Long = computeGcTime()
    var estimatedSize = 0L
    if (usingMap) {
      estimatedSize = map.estimateSize()
//      if (maybeSpill(map, estimatedSize)) {
      if (maybeSpillARC(map, estimatedSize, kvflag, context.partitionId())) {
        var endGcTime: Long = computeGcTime() - startGcTime
        logError("ARC kv1: " + kvcount + ", gc time: " + endGcTime + ", spill time: " +
          spillRunTime + " in task " + context.partitionId())
        if (endGcTime > spillRunTime * 0.1) {
          SparkEnv.kvThreshold = (kvcount / 2).asInstanceOf[Long]
          if (SparkEnv.kvThreshold < SparkEnv.lowerbound) {
            SparkEnv.kvThreshold = SparkEnv.lowerbound
          }
        } else if (endGcTime == 0) {
          SparkEnv.kvThreshold = (kvcount * 1.25).asInstanceOf[Long]
        }
        kvcount = 0
...
//      if (maybeSpill(buffer, estimatedSize)) {
      if (maybeSpillARC(buffer, estimatedSize, kvflag, context.partitionId())) {
        var endGcTime: Long = computeGcTime() - startGcTime
        logError("ARC kv2: " + kvcount + ", gc time: " + endGcTime + ", spill time: " +
          spillRunTime + " in task " + context.partitionId())
        if (endGcTime > spillRunTime * 0.1) {
          SparkEnv.kvThreshold = (kvcount / 2).asInstanceOf[Long]
          if (SparkEnv.kvThreshold < SparkEnv.lowerbound) {
  ...
  override protected[this] def spill(collection: WritablePartitionedPairCollection[K, C]): Unit = {
    ...
    var success = false
    try {
      val spillStart = System.currentTimeMillis()
      ...
      success = true
      spillRunTime = System.currentTimeMillis() - spillStart
    } finally {
      if (!success) {

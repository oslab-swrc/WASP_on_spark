...
private[spark] trait Spillable[C] extends Logging {
  ...
  protected def maybeSpillARC(collection: C, currentMemory: Long,
      kvflag: Boolean, taskId: Int): Boolean = {
    var shouldSpill = false
    if (elementsRead % 32 == 0 && currentMemory >= myMemoryThreshold) {
      // Claim up to double our current memory from the shuffle memory pool
      val amountToRequest = 2 * currentMemory - myMemoryThreshold
    ...
    shouldSpill = shouldSpill || _elementsRead > numElementsForceSpillThreshold || kvflag
    // Actually spill
    if (shouldSpill) {
      _spillCount += 1
      logSpillage(currentMemory)
